...
export default function AdminUsersIndex() {
  ...
  const resetFilters = () => {
    setQ("");
    setRoleFilter("any");
    router.get("/admin/roles", {}, { preserveState: false, replace: true });
  };
      ...
      <div className="rounded-2xl border border-slate-700/80 bg-gradient-to-b from-slate-900/90 to-slate-950/95 p-4 shadow-lg shadow-black/50 backdrop-blur">
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400">
              Manage which users are admins or students.
            </p>
          </div>
        </div>
        ...
        {users.links && users.links.length > 1 && (
          <div className="mt-4 text-xs text-slate-400">
            Page{" "}
            <span className="text-slate-200">{users.current_page}</span> of{" "}
            <span className="text-slate-200">{users.last_page}</span>
          </div>
        )}
      </div>
    </AuthenticatedLayout>
