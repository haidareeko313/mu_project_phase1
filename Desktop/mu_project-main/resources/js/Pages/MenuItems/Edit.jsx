...
export default function MenuItemsEdit() {
  const { item } = usePage().props;

  const { data, setData, post, processing, errors } = useForm({
    _method: "PUT",              // 👈 override to hit the PUT route
    name: item.name ?? "",
    price: item.price ?? "",
    stock_qty: item.stock_qty ?? 0,
    is_active: item.is_active ?? true,
    image: null,                 // optional new file
  });

  const submit = (e) => {
    e.preventDefault();

    post(`/menuitems/${item.id}`, {
      forceFormData: true,       // 👈 send as multipart/form-data (needed with file)
      preserveScroll: true,
    });
  };
          ...
          {item.image_url && (
            <div>
              ...
              <img
                src={item.image_url}
                alt={item.name}
                className="mb-2 h-24 w-32 rounded border border-slate-700 object-cover"
              />
            </div>
          )}
